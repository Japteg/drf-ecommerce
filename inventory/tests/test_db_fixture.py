...
def test_inventory_db_product_dbfixture(
    db, db_fixture_setup, id, web_id, name, slug, description, is_active, created_at,
    updated_at,
):
...
def test_inventory_db_product_insert_data(
    ...
    # these category ids are passed in post generation method defined in product factory
...
@pytest.mark.dbfixture
@pytest.mark.parametrize(
    "id, sku, upc, product_type, product, brand, is_active, retail_price, store_price, "
    "sale_price, weight, created_at, updated_at",
    [
        (
            1,
            "7633969397",
